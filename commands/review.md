...
#### 7. 设计质量（乔布斯简洁原则 + OPC 自动化原则）

- **简洁性（Jobs）**：是否引入了不必要的抽象层、配置项、参数？能否用更少的代码表达同样的意图？
- **API 最小面（Jobs）**：新增的公共接口/导出符号是否都有真实调用方？未使用的导出应删除而非保留
- **范围蔓延（Jobs）**：是否实现了任务边界外的"顺手"功能？拆解时已声明"不做"的功能是否被偷偷加回？（参照 `docs/task-breakdown-*.md` 的「聚焦决策」表）
- **手动操作残留（OPC）**：是否引入了需要人工执行的步骤（手动迁移、手动配置、手动重启）？这些应转化为脚本/自动化
- **流程冗余（OPC）**：是否新增了无法证明价值的流程步骤（多余的 staging 环境、多余的人工 review 节点）？

发现违反此维度的问题时，category 标记为 `design-quality`，severity 通常 medium（除非引入大量手动操作或显著膨胀，则 high）。

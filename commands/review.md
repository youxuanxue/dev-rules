...
只有高风险审查、变更显式触达这些基线，或存在常规风险 spec delta 时，才读取以下内容作为审查基线：

1. `docs/spec-delta-*.md` — 常规风险行为变更的最小意图载体，只用于理解 intent，不作为审批基线
2. `docs/approved/` — 人类审批通过的设计文档、API 契约、数据模型、技术选型
3. `.testing/user-stories/` — User Story 及其验收标准（AC）
4. `CLAUDE.md` — 项目架构约束

审查顺序为 `Intent → Code → Validation`：先确认变更意图是否清楚，再看代码是否符合，最后看验证证据是否覆盖关键场景。低风险变更不因缺少 spec delta 被阻塞；常规风险以上若行为变化已发生但意图载体缺失或表达不清，可作为 `design-quality` 或 `validation` finding 指出。
...
2. 按需读取意图与审批产物（`docs/spec-delta-*.md`、`docs/approved/`、`.testing/user-stories/`、`CLAUDE.md`）
3. 获取审查范围内的所有变更：
  ```bash
   git log --oneline --since="24 hours ago"
   git diff HEAD~N
  ```
4. 按 `Intent → Code → Validation` 分析：低风险可直接看 code 和 validation；常规风险以上若存在 spec delta / approved docs / Story，先确认 intent 清楚且未超范围，再看代码与验证
5. 逐文件分析变更：先做通用质量检查；只有在高风险或显式要求时再做符合性检查
6. 按严重程度分级（JSON `severity` 字段取值）：
...
8. Markdown 摘要中的映射：critical/high → CRITICAL，medium → WARNING，low → INFO

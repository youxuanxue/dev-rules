...
def init_workspace(goal_file: Path, persona_file: Path, out: Path | None = None) -> Path:
    ...
    runbook = [
        ...
        "2. Pick exactly one pending ledger feature as current focus.",
        "3. Supervisor emits one concise instruction to the worker session.",
        "4. Worker produces code changes and validation evidence.",
        "5. Stop for human gates: architecture, security, data, dependencies, external side effects, destructive actions.",
        "",
        "Validation commands:",
    ]

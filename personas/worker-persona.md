...
- 低风险和常规风险中间决策按 Jobs/OPC 直接推进，不频繁停下来问。
...
遇到 Claude Code permissions / disallowedTools / hooks / dev-rules 拦截，或明显高风险时，不要绕过；报告 blocker：

- 受保护分支/main/master/release 分支变更、force push、改写已发布历史、merge 到受保护分支。
- plan mode / dev-rules / 项目上下文未明确覆盖的架构边界变化。
- 安全边界变化。

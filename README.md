...
每个载体承载的哲学（与本仓库根的 `digital-clone-research.md` §10 表保持同步）：
...
详细原则说明见本仓库根的 `digital-clone-research.md` §〇「两个哲学基石」。
...
└── .local-projects                                   ← 本机落地映射（gitignored，TSV: git_url\tabs_path）
```

**为什么注册表拆成两个文件？**
跨机器同一个项目克隆在不同的本地路径（甚至某些机器上根本没克隆）。`.registered-projects` 回答「哪些项目属于这个 dev-rules 生态」（跨机器一致），`.local-projects` 回答「在这台机器上分别落在哪儿」（per-machine）。fan-out 时按 `(注册项目 ∩ 本机已落地)` 工作，未落地项目静默跳过；换机器只需在该机器各项目里跑一次 `sync.sh --local` 重建本机映射。
...
软规则与硬检查的完整映射定义在本仓库根的 `digital-clone-research.md §六.½`，接入步骤见 `rules/dev-rules-convention.mdc`。本仓库交付的工件清单：
...
| `sync.sh --push`                                         | push submodule + ~/Codes pull --ff-only + fan-out 到本机所有已落地项目（编辑者主动同步，原子动作）                                            |
| `sync.sh --pull`                                         | 远端 → ~/Codes → 本机已落地项目 fan-out（LaunchAgent 与手动救场用）                                                                    |
| `.registered-projects` + `.local-projects`               | 跨机器项目注册表（git-tracked, `name\tgit_url`）+ 本机落地映射（gitignored, `git_url\tabs_path`）；`--local` 自动维护两者                       |
| `schemas/review.schema.json`                             | 由 ajv / check-jsonschema 消费；calibrate 入口校验                                                                            |
| `schemas/skill.schema.json`                              | 通用 Agent Skill manifest 契约（origin: zw-brain §3，已通用化）；任何注册到 Agent 平台的 Skill 必须 validate 通过                             |
...
4. 之后 `verify-rules.sh` 的 LaunchAgent 实装段会强制要求该机器上必须实装，否则 exit 1
...
dev-rules/verify-rules.sh                                          # <!-- stat:verify-rules-checks -->8<!-- /stat --> 段强约束自检

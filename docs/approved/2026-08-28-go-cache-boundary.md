...
零散、低频的 Go workflow 要么消费同一 restore-only action，要么不使用 build cache；不得重新启用 setup-go 的隐式 cache。
...
禁止包含 `github.run_id`、日期、branch 或绝对 checkout path。内容未变化时精确命中且不重新上传；内容变化时 restore 上一代、编译 delta、保存新内容 key。所有 CI `GOFLAGS` 必须包含 `-trimpath`，profile 特异 flag 继续作为 Go 自身 action ID 的一部分。

唯一写入者必须显式按 `actions/cache/restore -> build -> actions/cache/save -> prune` 排序，不能使用在 Job 尾部才保存的 combined action；否则 prune 无法观察刚保存的新代。restore consumer 永远使用 restore-only action。
...
- add mechanical tests that reject `setup-go cache: true`, large `github.run_id` keys, unbounded writers and missing `-trimpath`;
...
- merge the new restore-only consumers and main writer;
- observe two successful main warm generations and required CI restores;
- inventory legacy cache by exact ID, prefix, ref, size and last access;
- request approval for that resolved deletion set;
- delete only the approved legacy Release, benchmark, setup-go and superseded Go family caches;
- verify the active working set is at or below `6 GiB`.
...
- every Go job uses the exact `go.mod` version and effective `-trimpath`;

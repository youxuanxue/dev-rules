...
| 公共契约删除必须有显式说明锚点（`agent-contract-enforcement.mdc` / `product-dev.mdc`） | `dev-rules/scripts/check_contract_deletion_notice.py` → `scripts/preflight.sh` 段 10 | pre-commit / CI | exit 1；阻断静默删约 |
| 分层依赖不可反转（`agent-contract-enforcement.mdc`） | `dev-rules/scripts/check_layer_dependency_inversion.py`（读取 `.preflight/layer-deps.json`）→ `scripts/preflight.sh` 段 11 | pre-commit / CI（有配置时） | exit 1；阻断反向依赖 |
| 高风险改动必须绑定审批锚点（`product-dev.mdc`） | `dev-rules/scripts/check_high_risk_anchor.py` → `scripts/preflight.sh` 段 12 | pre-commit / CI | exit 1；阻断无锚点高风险改动 |
| release 敏感语境提交禁止 skip-ci marker（`product-dev.mdc`） | `dev-rules/scripts/check_release_skip_ci_safety.py` → `scripts/preflight.sh` 段 13 | pre-commit / CI（release 语境） | exit 1；阻断流水线静默跳过风险 |
| GitHub Actions workflow 的 job-level `if` 禁止引用 `env.*` 上下文（该上下文在 job-level if 求值时尚未建立；违反时整份 workflow HTTP 422 解析失败，所有 job 拒绝启动）                              | 项目级 `scripts/check-workflow-job-if-env.py`（扫描 `.github/workflows/*.yml`，发现 jobs.*.if 中出现 `env.*` 即 exit 1）→ `scripts/preflight.sh`；step-level if 中的 `env.*` 仍允许，不被 flag | pre-commit / CI | exit 1；整份 workflow 解析失败，所有 job 不启动，发版通道被锁；fix：把 `env.*` 展开为 `vars.*` 或 `github.event.inputs.*` |
| `claude -p` headless 调用契约：无 `--output` flag（正确形式：`2>&1 \| tee /tmp/out.txt` + `set -o pipefail`）；缺 `--allowedTools` 时 agent 以 `--permission-mode default` 运行，在 CI 无人值守环境中拒绝所有工具调用并静默完成（exit 0，零产出，无报错——最难感知的失败模式）                              | `dev-rules/templates/cloud-agent-bootstrap.sh --check`（工具 + secret 层）；workflow YAML 的调用 pattern（`--allowedTools`、`set -o pipefail`、`tee`）靠 PR review 兜底 | preflight 段 9 / cloud-agent install / PR review | 工具/secret 缺失 exit 1；YAML pattern 错误 = agent 完成但零产出，或 agent 失败但 CI step 绿灯 |

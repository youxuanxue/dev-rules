...
| Claude Code 全局宪法（`~/.claude/CLAUDE.md`）与 Codex 全局指令（`~/.codex/AGENTS.md`）不是手维护的孤儿文件 (`dev-rules-convention.mdc`)                                    | `dev-rules/global/CLAUDE.md` + `sync.sh` 自动 symlink（两端同源）                                               | 跨会话持续                | 链接断裂时 sync.sh `--status` 标 ⚠；`--check` exit 1 |
| Home 层 `~/.cursor/skills` 必须指向 dev-rules canonical mirror 的 `.cursor/skills`，否则 Claude/Codex/Antigravity 全局技能链路会从源头断开 (`dev-rules-convention.mdc`) | `dev-rules/sync.sh` 自动 symlink + `sync.sh --check` 的 home Cursor skills drift 检查 | 跨会话持续 | 链接缺失 / 真实目录 / 错误目标 / 源不存在时 `sync.sh --check` exit 1 |
...
| Codex 消费端经 `<repo>/AGENTS.md` 的 dev-rules 受管块复用宪法/规则/技能索引；该块确定性生成、禁止手工编辑（`dev-rules-convention.mdc`） | `dev-rules/scripts/gen_codex_agents.py`（生成 + `--check`，`--self-test` 受 `verify-rules.sh` 守护）→ `scripts/preflight.sh`（codex AGENTS.md managed block） | pre-commit / CI（有受管块时） | exit 1；阻断 Codex 项目指令漂移 |
| Agent Skill `description` 不超过 Codex 加载上限（1024 字符），否则 Codex 静默丢弃该技能而 Cursor/Claude 无感（`dev-rules-convention.mdc`） | `dev-rules/scripts/check_codex_skill_limits.py`（`--self-test` 受 `verify-rules.sh` 守护）→ `scripts/preflight.sh`（codex skill description length） | pre-commit / CI（有 `.cursor/skills` 时） | exit 1；阻断 Codex 复用时技能静默缺失 |

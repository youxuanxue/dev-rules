...
| 有 Web surface 的仓库中，后端服务/业务逻辑改动必须同 PR 对齐 Web 页面、配置、契约或 Story；确无影响时必须显式说明（`agent-contract-enforcement.mdc` / `product-dev.mdc`） | `dev-rules/scripts/check_web_surface_alignment.py`（项目可用 `.preflight/web-surface-alignment.conf` 覆写路径与 token）→ `scripts/preflight.sh`（web surface alignment） | pre-commit / CI | exit 1；阻断后端上线后 Web 缺配置/缺呈现 |
...
| `claude -p` headless 调用契约：无 `--output` flag（正确形式：`2>&1 \| tee /tmp/out.txt` + `set -o pipefail`）；缺 `--allowedTools` 时 agent 以 `--permission-mode default` 运行，在 CI 无人值守环境中拒绝所有工具调用并静默完成（exit 0，零产出，无报错——最难感知的失败模式）                              | `dev-rules/templates/cloud-agent-bootstrap.sh --check`（工具 + secret 层）；workflow YAML 的调用 pattern（`--allowedTools`、`set -o pipefail`、`tee`）靠 PR review 兜底 | preflight: cloud-agent consistency / cloud-agent install / PR review | 工具/secret 缺失 exit 1；YAML pattern 错误 = agent 完成但零产出，或 agent 失败但 CI step 绿灯 |

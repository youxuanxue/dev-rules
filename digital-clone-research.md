...
| `.reviews/*.json` 必须通过 `schemas/review.schema.json`（`/user:review` 输出契约不能漂） | `dev-rules/scripts/check_review_record.py` → `scripts/preflight.sh`（review record schema） | pre-commit / CI（有 `.reviews/` 时） | exit 1；阻断 reviewer 输出形态漂移 |
